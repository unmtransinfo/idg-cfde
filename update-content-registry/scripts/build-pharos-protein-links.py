...
# CV for proteins are UniProt IDs. Map to Pharos URLs via file.
def make_markdown(cv_id):
    return f"""
## Pharos URLs

* Pharos: [https://pharos.nih.gov/targets/{cv_id}](https://pharos.nih.gov/targets/{cv_id}) 
* UniProtKB: [https://www.uniprot.org/uniprotkb/{cv_id}/entry](https://www.uniprot.org/uniprotkb/{cv_id}/entry)
"""
...
def main():
    ...
    # CV for proteins are UniProt IDs. Map to Pharos URLs via file.
    # tcrd_targets.tsv produced by BioClients.idg.tcrd.Client listTargets.
    tsvfile = "data/inputs/tcrd_targets.tsv"
