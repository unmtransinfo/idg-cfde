...
```{r}
LS_Mapping <- read_delim(paste0(Sys.getenv()["HOME"], "/../data/CFDE/DODGETox/LS_Mapping_PubChem.tsv"), "\t")
setDT(LS_Mapping)
LS_CIDs <- unique(LS_Mapping[, CID])
```
...
## LeadScope, LINCS, ReproTox-Blood, ReproTox-Cardiovascular, ReproTox-CNS
...
X <- list(
	...
	LINCS = SM_LINCS_CIDs,
	LeadScope = LS_CIDs)
display_venn(X, fill = c("#0073C2FF", "#EFC000FF", "#868686FF", "pink", "cyan"))
```
...
X <- list(
	...
	ReproTox_Cardiovascular = ReproTox_data_Cardiovascular_CIDs, 
	ReproTox_CNS = ReproTox_data_CNS_CIDs)
display_venn(X, fill=c("#0073C2FF", "#EFC000FF", "#868686FF"))
```

## LeadScope, LINCS, ReproTox-ALL
...
ReproTox_ALL = unique(c(ReproTox_data_Blood_CIDs, ReproTox_data_Cardiovascular_CIDs, ReproTox_data_CNS_CIDs)),
	LINCS = SM_LINCS_CIDs,
	LeadScope = LS_CIDs)
display_venn(X, fill = c("#868686FF", "pink", "cyan"))
```
