...
sims[, tanimoto_similarity_max := max(tanimoto_similarity), by=mol_id_a]
sims <- sims[tanimoto_similarity==tanimoto_similarity_max]
sims[, tanimoto_similarity_max := NULL]
setorder(sims, mol_id_a)
# Keep only first
sims[, `:=`(mol_id_b=first(mol_id_b), tanimoto_similarity=first(tanimoto_similarity)), by=mol_id_a]
sims <- unique(sims)

qs <- quantile(sims[["tanimoto_similarity"]], c(0, .25, .75, seq(.8, 1, .01)))
writeLines(sprintf("Tanimoto %4s-ile: %9.1f", names(qs), qs))

plot_ly(type="histogram", x=sims[ ,tanimoto_similarity])  %>%
  layout(title=sprintf("Tanimoto-RDKitFp similarity distribution<br>ReproTox-not-LINCS to most-similar LINCS compound"), margin=list(t=80, b=60, l=30),
